<?php
    ...
    $stmt = $pdo->prepare("INSERT INTO produtos(nome, id_imagem, preco, categoria, descricao, info_tecnica)
    VALUES (:nome_produto, :id_imagem,  :produto_preco, :produto_categoria, :produto_descricao, :produto_info_tec)");
    $stmt->bindParam(':nome_produto', $nome_produto);
    $stmt->bindParam(':id_imagem', $nome_produto);
    ...
    $targetDir = "../prodigy_imagens/product_imgs/";
    $targetFile = $targetDir . basename($_FILES["photo"]["name"]);
    $upload0k = 1;
    $imageFileType = strtolower(pathinfo($targetFile, PATHINFO_EXTENSION));

    if ($upload0k = 0){
        echo "The img was not uploaded";
    } else {
