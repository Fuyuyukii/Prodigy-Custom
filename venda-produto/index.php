            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="card card-padding">
                        <div class="card-body">
                            <input type="text" name="" id="pesquisa" class="form-control text-center"
                                placeholder="Pesquisa">

                            <div class="col-md-12 d-flex justify-content-between pt-4 pb-4">
                                <button class="button-primario" data-bs-toggle="offcanvas" href="#offcanvasExample"
                            ...
                            <div class="heightfix">
                                ...
                                $comando = $pdo->prepare("select * from carro_produto inner join produtos on carro_produto.produto_id = produtos.id_produto");
                                $comando->execute();
...
<script>
    $(function () {
    $("#InsereImagemCarro").change(function () {
        $(this).css('display', 'none')
        $('#ImagemCarroInserida').css('display', 'block')
        $('.labelImagem').css('display', 'none')
        if (this.files && this.files[0]) {
            var reader = new FileReader();
            reader.onload = function (e) {
                $('#ImagemCarroInserida').attr('src', e.target.result);
            }
            reader.readAsDataURL(this.files[0]);
        }
    })
    $('#ImagemCarroInserida').on('click', function(){
        $('#InsereImagemCarro').trigger('click')
        if (this.files && this.files[0]) {
            var reader = new FileReader();
            reader.onload = function (e) {
                $('#ImagemCarroInserida').attr('src', e.target.result);
